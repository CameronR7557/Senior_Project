...
/* Variables */
static uint16_t motor1_dmabuffer[DSHOT_DMA_BUFFER_SIZE];
static uint16_t motor2_dmabuffer[DSHOT_DMA_BUFFER_SIZE];
static uint16_t motor3_dmabuffer[DSHOT_DMA_BUFFER_SIZE];
static uint16_t motor4_dmabuffer[DSHOT_DMA_BUFFER_SIZE];
...
static void dshot_prepare_dmabuffer(uint16_t* motor_dmabuffer, uint16_t value);
...
static void dshot_dma_tc_callback(DMA_HandleTypeDef *hdma)
{
	...
	if (hdma == htim->hdma[TIM_DMA_ID_CC1])//Cahnge these I think
	{
...
static uint16_t dshot_prepare_packet(uint16_t value)//Should be same as other
{
	uint16_t packet;
...
static void dshot_prepare_dmabuffer(uint16_t* motor_dmabuffer, uint16_t value)//Also should be same as other
{
	uint16_t packet;
