...
//CRC32 Calculations: On all data in packet aside from CRC itself. Used https://crccalc.com/ -> CRC-32/MPEG-2 (results are flipped, so 0x3B6245AC -> 0xAC45623B)
...
{
	uint8_t payload[10] = {0xF5, 0xE1, 0x00, 0x00, 0x00, 0x00, 0xA5, 0x8D, 0x89, 0xA7};//Mode 0 = Single, 1(?) = Continuous
	HAL_UART_Transmit(&huart5, payload, 10, 1000);
	HAL_UART_Receive(&huart5, buffer, 12, 1000);
}
...
{
	uint8_t payload[10] = {0xF5, 0xE1, 0x01, 0x00, 0x00, 0x00, 0x12, 0x17, 0xE4, 0x7B};//Mode 0 = Single, 1(?) = Continuous
	HAL_UART_Transmit(&huart5, payload, 10, 1000);
	HAL_UART_Receive(&huart5, buffer, 12, 1000);
}
...
{
	uint8_t payload[10] = {0xF5, 0x65, 0, 0, 0, 0, 0x9A, 0x08, 0xE9, 0x8A};
	HAL_UART_Transmit(&huart5, payload, 10, 1000);
	HAL_UART_Receive(&huart5, buffer, 12, 1000);
}
//From: https://barrgroup.com/blog/crc-series-part-3-crc-implementation-code-cc
