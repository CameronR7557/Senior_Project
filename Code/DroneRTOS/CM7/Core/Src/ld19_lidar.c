...
void ParsePacket(uint8_t* packet, LiDARFrameTypeDef* frame)
{
	...
	frame->ver_len = packet[1];
	frame->speed = packet[3] | packet[2];
	frame->start_angle = packet[4] | packet[5];
	for(uint8_t i = 0; i < 12; ++i)
	{
		frame->point[i].distance = packet[7 + i*3] | packet[6 + i*3];
		frame->point[i].intensity = packet[8 + i*3];
	}
	frame->end_angle = packet[43] | packet[42];
	frame->timestamp = packet[45] | packet[45];
	frame->crc8 = packet[46];
}
...
void getLiDARMeasurements(uint8_t* packet, uint8_t* dataPacket, uint16_t* fullDataPacket)
{
	...
	if(packet != NULL && dataPacket != NULL)
	{
		...
		for(uint8_t i = 4; i < 16; ++i)//organize lidar data (scales values so that dists 0 - 2000mm are scaled to 0 - 120 which is the number of pixels that the LCD has in on direction)
		{
